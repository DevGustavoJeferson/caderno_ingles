# 🧩 Exercícios

> Pasta dedicada à prática de inglês por meio de exercícios encontrados em sites, plataformas e materiais online gratuitos.

---

## 📝 Descrição

- Esta seção tem como objetivo reunir **exercícios práticos** encontrados em diferentes fontes da internet.  
- Cada exercício incluirá o **enunciado original**, a **resolução** e, quando possível, uma **reflexão linguística** sobre o tema abordado.  

> 📍 O foco aqui é **praticar leitura, escrita e interpretação em inglês**, reforçando o conteúdo visto nas seções do [Curso(Udemy)](Ingles\Curso(Udemy)).

### ⚠️ Importante

- As anotações principais terão números com “.0” (ex: `1.0`, `2.0`), representando os temas principais.  
  - As anotações derivadas (ex: `1.1`, `1.2`) servem como complemento ou variação do mesmo tema, permitindo mais prática e aprofundamento.

---

## 🌐 Fontes e Plataformas

- [BBC Learning English](https://www.bbc.co.uk/learningenglish)
- [Duolingo Stories](https://www.duolingo.com)
- [Perfect English Grammar](https://www.perfect-english-grammar.com)
- [Cambridge English Practice](https://www.cambridgeenglish.org/learning-english/)
- [British Council – Learn English](https://learnenglish.britishcouncil.org/)
- [ISLCorrective](https://en.islcollective.com/)
- De acordo com o crescimento do projeto, mais fontes poderão ser acrescentadas

---

## 🧭 Propósito

> “A prática constante transforma o conhecimento em fluência.”

- Este espaço foi criado para **aplicar, revisar e registrar o progresso contínuo** no aprendizado de inglês, unindo **teoria (do curso Udemy)** e **prática (dos exercícios encontrados online)**.

---

## 🧠 Ferramentas Utilizadas

- 📝 Editor: **Visual Studio Code**  
- 🌐 Pesquisa e exercícios: **sites educacionais de inglês**  
- 🪟 Sistema operacional: **Windows**

---

## 📖 Bibliografia / Referências

*(Esta seção será atualizada conforme novos sites e materiais forem adicionados.)*

---

> 💬 *“Practice makes perfect.”*
